...
func (c *Client) UploadLargeFile(filePath, bucket, object string, options map[string]string) (map[string]string, error) {
	...
	//初化化上传
	initUpload, err := c.InitUpload(bucket, object, map[string]string{"disposition": options["disposition"]})
	if err != nil {
		return nil, err
	}
	...
	for {
		...
		go func(partNum int, fd *os.File) {
			...
			for i := 0; i < c.maxRetryNum; i++ {
				//body := io.NewSectionReader(fd, int64(offset), int64(num))
...
func (c *Client) InitUpload(bucket, object string, options map[string]string) (*InitUploadResult, error) {
	addr := fmt.Sprintf("http://%s%s/%s?uploads", bucket, c.host, object)
	method := "POST"
	...
	LF := "\n"
	headers["Authorization"] = c.sign(method+LF, headers, bucket, fmt.Sprintf("%s?uploads", object))
...
func (c *Client) UploadPart(body *bytes.Reader, bucket, object string, partNumber int, uploadID string) (map[string]string, error) {
	addr := fmt.Sprintf("http://%s%s/%s?partNumber=%d&uploadId=%s", bucket, c.host, object, partNumber, uploadID)
	method := "PUT"
	contentType := mime.TypeByExtension(path.Ext(object))
...
func (c *Client) CompleteUpload(body []byte, bucket, object, uploadID string, objectSize int) (map[string]string, error) {
	addr := fmt.Sprintf("http://%s%s/%s?uploadId=%s", bucket, c.host, object, uploadID)
	method := "POST"
	contentType := mime.TypeByExtension(path.Ext(object))
	...
	headers["Authorization"] = c.sign(method, headers, bucket, fmt.Sprintf("%s?uploadId=%s", object, uploadID))
	headers["Content-Length"] = contentLength
