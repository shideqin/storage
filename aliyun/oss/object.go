...
func (c *Client) UploadFile(filePath, bucket, object string, options map[string]string) (map[string]string, error) {
	...
	return c.Put(body, bucket, object, map[string]string{"disposition": options["disposition"]})
}
...
func (c *Client) Put(body []byte, bucket, object string, options map[string]string) (map[string]string, error) {
	...
	return map[string]string{
		...
		"Location":         "http://" + bucket + c.host + "/" + object,
		"Size":             contentLength,
		"Bucket":           bucket,
		"ETag":             res["Etag"],
		"Key":              object,
...
func (c *Client) CopyFromHlsFile(bucket, prefix, source string, options map[string]string) (map[string]int, error) {
	...
	for {
		...
		go func(objectInfo map[string]string) {
			...
			if !isSkipped {
				...
				isCopySuccess := false
				for i := 0; i < c.maxRetryNum; i++ {
					res, err := c.Copy(bucket, object, sourceObject, map[string]string{"disposition": sourceHead["Content-Disposition"]})
					if err != nil {
						continue
					}
					isCopySuccess = true
	...
	if finish > 0 {
		...
		newObject := prefix + path.Base(sourceObject)
		res, err := c.Put([]byte(newM3u8Content), bucket, newObject, map[string]string{"disposition": ""})
		if err != nil {
			return nil, err
		}
		if res["StatusCode"] != "200" {
	...
	for {
		...
		go func(fileName string) {
			...
			if !isSkipped {
				...
				for i := 0; i < c.maxRetryNum; i++ {
					res, err := c.Put(body, bucket, object, map[string]string{"disposition": fileName})
					if err != nil {
						continue
					}
...
func (c *Client) ListObject(bucket string, options map[string]string) (*ListObjectResult, error) {
	...
	addr := "http://" + bucket + c.host + "/?" + strings.TrimLeft(param, "&")
	method := "GET"
	date := time.Unix(time.Now().Unix()-8*3600, 0).Format(c.dateTimeGMT)
	headers := map[string]string{
		"Date": date,
	...
	for {
		...
		go func(objectInfo ListObjectContents) {
			...
			if !isSkipped {
				...
				isCopySuccess := false
				for i := 0; i < c.maxRetryNum; i++ {
					res, err := c.Copy(bucket, object, sourceObject, map[string]string{"disposition": ""})
					if err != nil {
						continue
					}
					isCopySuccess = true
...
LIST:
	...
	for {
		...
		go func(fileNum int, body string) {
			defer wg.Done()
			addr := "http://" + bucket + c.host + "/?delete"
			method := "POST"
			date := time.Unix(time.Now().Unix()-8*3600, 0).Format(c.dateTimeGMT)
			contentLength := strconv.Itoa(len(body))
			...
			headers["Authorization"] = c.sign(method, headers, bucket, "?delete")
			headers["Content-Length"] = contentLength
